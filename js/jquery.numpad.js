    ...
    $.fn.numpad=function(options){
		...
		return this.each(function(){
			...
			if ($('#'+id).length == 0) {
				...
				nmpd.append($(options.backgroundTpl).addClass('nmpd-overlay').click(function(){nmpd.close(false, true);}));
			...
			*/
			nmpd.open = function(target, initialValue){
				...
				$('#'+id+' .done').off('click');
				$('#'+id+' .done').on('click', function(){ nmpd.close(target); });
